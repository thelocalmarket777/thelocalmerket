    ...
    RemoteServices.getnotificationHistory()
      .then((response) => {
        const allNotifications: Notification[] = [
          ...response.data.order.map(n => ({ 
            ...n, 
            type: 'order' as const,
            order_items: n.orders_items
          })),
          ...response.data.promotion.map(n => ({ ...n, type: 'promotion' as const })),
          ...response.data.system.map(n => ({ ...n, type: 'system' as const }))
          ...
          <div className="space-y-4">
            {filteredNotifications.length > 0 ? (
              filteredNotifications.map((notification) => {
                // Determine background color based on status or read state
                ...
                return (
                  <div 
                    key={notification.id}
                    className={`p-4 rounded-lg border shadow-sm hover:shadow transition-all ${bgColor}`}
                                  ...
                                  <div key={index} className="flex items-center p-2 bg-white rounded-lg border border-gray-100">
                                    ...
                                    <div className="ml-3 flex-1">
                                      <div className="flex justify-between">
                                        ...
                                        <span className="text-sm font-medium text-gray-900">${parseFloat(item.price).toFixed(2)}</span>
                                      </div>
                                ...
                                {notification.total_amount && (
                                  <div className="flex justify-end mt-2">
                                    <span className="text-sm font-semibold text-gray-900">
                                      Total: ${parseFloat(notification.total_amount).toFixed(2)}
                                    </span>
                                  </div>
                                )}
                              </div>
                    ...
                    {notification.status && (
                      <div className="mt-3 flex justify-end">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                          notification.status.toLowerCase() === 'cancelled' ? 'bg-red-100 text-red-800' :
                          notification.status.toLowerCase() === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                          notification.status.toLowerCase() === 'delivered' ? 'bg-green-100 text-green-800' :
                          notification.status.toLowerCase() === 'shipped' ? 'bg-blue-100 text-blue-800' :
